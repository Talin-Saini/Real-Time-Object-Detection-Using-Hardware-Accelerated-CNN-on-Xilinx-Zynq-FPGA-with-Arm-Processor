...
# 24 "./accelerator.h"
__attribute__((sdx_kernel("cnn_accelerator", 0))) void cnn_accelerator(
    data8 *input,
    data8 *weights8,
    data6 *weights6,
    data4 *weights4,
...
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=cnn_accelerator
# 16 "accelerator.cpp"
...
#pragma HLS INTERFACE s_axilite port=return
 ...
 static data8 in_buf[16][8 +3 -1][8 +3 -1];
    static acc_t out_buf[16][8][8];
...
#pragma HLS bind_storage variable=out_buf type=ram_2p impl=bram
 ...
 VITIS_LOOP_39_1: for(int to=0; to<16; to++)
    VITIS_LOOP_40_2: for(int tr=0; tr<8; tr++)
    VITIS_LOOP_41_3: for(int tc=0; tc<8; tc++){
#pragma HLS PIPELINE II=1

 acc_t sum = 0;

        VITIS_LOOP_46_4: for(int ti=0; ti<16; ti+=8){
...
#pragma HLS ARRAY_PARTITION variable=partial complete
 ...
 VITIS_LOOP_58_5: for(int k=0;k<8;k++){
#pragma HLS UNROLL
 act[k] = input[(ti+k)*H*W + tr*W + tc];
            ...
            VITIS_LOOP_70_6: for(int k=0;k<8;k++) sum += partial[k];
        }
