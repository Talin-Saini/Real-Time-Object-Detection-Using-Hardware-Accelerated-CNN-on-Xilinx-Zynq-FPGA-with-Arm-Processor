...
// ==============================================================
`timescale 1ns/1ps
module cnn_accelerator_control_s_axi
#(parameter
    ...
    output wire [63:0]                   weights8,
    output wire [63:0]                   weights6,
    output wire [63:0]                   weights4,
    output wire [63:0]                   weights2,
    output wire [63:0]                   output_r,
    output wire [31:0]                   Cin,
    output wire [31:0]                   Cout,
    output wire [31:0]                   H,
    output wire [31:0]                   W,
    output wire [31:0]                   precision,
...
localparam
    ...
    ADDR_WEIGHTS8_DATA_0  = 7'h1c,
    ADDR_WEIGHTS8_DATA_1  = 7'h20,
    ADDR_WEIGHTS8_CTRL    = 7'h24,
    ADDR_WEIGHTS6_DATA_0  = 7'h28,
    ADDR_WEIGHTS6_DATA_1  = 7'h2c,
    ADDR_WEIGHTS6_CTRL    = 7'h30,
    ADDR_WEIGHTS4_DATA_0  = 7'h34,
    ADDR_WEIGHTS4_DATA_1  = 7'h38,
    ADDR_WEIGHTS4_CTRL    = 7'h3c,
    ADDR_WEIGHTS2_DATA_0  = 7'h40,
    ADDR_WEIGHTS2_DATA_1  = 7'h44,
    ADDR_WEIGHTS2_CTRL    = 7'h48,
    ADDR_OUTPUT_R_DATA_0  = 7'h4c,
    ADDR_OUTPUT_R_DATA_1  = 7'h50,
    ADDR_OUTPUT_R_CTRL    = 7'h54,
    ADDR_CIN_DATA_0       = 7'h58,
    ADDR_CIN_CTRL         = 7'h5c,
    ADDR_COUT_DATA_0      = 7'h60,
    ADDR_COUT_CTRL        = 7'h64,
    ADDR_H_DATA_0         = 7'h68,
    ADDR_H_CTRL           = 7'h6c,
    ADDR_W_DATA_0         = 7'h70,
    ADDR_W_CTRL           = 7'h74,
    ADDR_PRECISION_DATA_0 = 7'h78,
    ADDR_PRECISION_CTRL   = 7'h7c,
    WRIDLE                = 2'd0,
    WRDATA                = 2'd1,
    WRRESP                = 2'd2,
    WRRESET               = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_weights8 = 'b0;
    reg  [63:0]                   int_weights6 = 'b0;
    reg  [63:0]                   int_weights4 = 'b0;
    reg  [63:0]                   int_weights2 = 'b0;
    reg  [63:0]                   int_output_r = 'b0;
    reg  [31:0]                   int_Cin = 'b0;
    reg  [31:0]                   int_Cout = 'b0;
    reg  [31:0]                   int_H = 'b0;
    reg  [31:0]                   int_W = 'b0;
    reg  [31:0]                   int_precision = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_WEIGHTS8_DATA_0: begin
                    rdata <= int_weights8[31:0];
                end
                ADDR_WEIGHTS8_DATA_1: begin
                    rdata <= int_weights8[63:32];
                end
                ADDR_WEIGHTS6_DATA_0: begin
                    rdata <= int_weights6[31:0];
                end
                ADDR_WEIGHTS6_DATA_1: begin
                    rdata <= int_weights6[63:32];
                end
                ADDR_WEIGHTS4_DATA_0: begin
                    rdata <= int_weights4[31:0];
                end
                ADDR_WEIGHTS4_DATA_1: begin
                    rdata <= int_weights4[63:32];
                end
                ...
                ADDR_CIN_DATA_0: begin
                    rdata <= int_Cin[31:0];
                end
                ADDR_COUT_DATA_0: begin
                    rdata <= int_Cout[31:0];
                end
                ADDR_H_DATA_0: begin
                    rdata <= int_H[31:0];
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_WEIGHTS8_DATA_0)
            int_weights8[31:0] <= (WDATA[31:0] & wmask) | (int_weights8[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_weights8[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_WEIGHTS8_DATA_1)
            int_weights8[63:32] <= (WDATA[31:0] & wmask) | (int_weights8[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_weights6[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_WEIGHTS6_DATA_1)
            int_weights6[63:32] <= (WDATA[31:0] & wmask) | (int_weights6[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_WEIGHTS4_DATA_0)
            int_weights4[31:0] <= (WDATA[31:0] & wmask) | (int_weights4[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_weights4[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_WEIGHTS4_DATA_1)
            int_weights4[63:32] <= (WDATA[31:0] & wmask) | (int_weights4[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_CIN_DATA_0)
            int_Cin[31:0] <= (WDATA[31:0] & wmask) | (int_Cin[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COUT_DATA_0)
            int_Cout[31:0] <= (WDATA[31:0] & wmask) | (int_Cout[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_precision[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_PRECISION_DATA_0)
            int_precision[31:0] <= (WDATA[31:0] & wmask) | (int_precision[31:0] & ~wmask);
    end
end
